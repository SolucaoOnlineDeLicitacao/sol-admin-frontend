    ...
    .container.tool.biddings.u-cf(v-else-if="proposalsCount > 0")
      .card.mb-2.u-cf.u-full-width(v-for="proposal in proposals")
        ...
        .row(v-if="proposal.status == 'coop_refused'")
          ...
          .button.button-success.u-pull-right(@click="confirmRefuse(proposal)")
            | {{ $t('.button.confirmRefuse') }}

        .row(v-else-if="proposal.status == 'coop_accepted'")
          .button.button-danger(@click="toggleReproveProposalOverlay(proposal)")
            | {{ $t('.button.reproveProposal') }}

          .button.button-success.u-pull-right(@click="confirmAccept(proposal)")
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        i18nScope: 'covenants.biddings.proposals.index',
    ...
    methods: {
      fetch() {
        ...
        return this.$http.get('/administrator/covenants/' + this.covenantId + '/biddings/' + this.biddingId + '/proposals', { params })
          .then((response) => {
      ...
      confirmAccept(proposal) {
        this.$http.patch('/administrator/proposals/' + proposal.id + '/accept')
          .then((response) => {
      ...
      confirmRefuse(proposal) {
        this.$http.patch('/administrator/proposals/' + proposal.id + '/refuse')
          .then((response) => {
            this.$notifications.clear()
      ...
      failProposal() {
        ...
        this.$http.patch('/administrator/proposals/' + this.proposal.id + '/fail', params)
          .then((response) => {
            this.$notifications.clear()
      ...
      updateRoute() {
        ...
        this.$router.replace({ name: 'globalProposals', query: data });
      },
